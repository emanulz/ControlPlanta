        ...
        <ul class="actionlist">
          {% for entry in admin_log %}
            <li class="{% if entry.is_addition %}addlink{% endif %}{% if entry.is_change %}changelink{% endif %}{% if entry.is_deletion %}text-error{% endif %}">
              ...
              {% spaceless %}
                {% if entry.is_addition %}{% trans 'agregado'|capfirst %}{% endif %}
                {% if entry.is_change %}{% trans 'cambiado'|capfirst %}{% endif %}
                {% if entry.is_deletion %}{% trans 'eliminado'|capfirst %}{% endif %}

                {% if entry.content_type %}
                  <span class="mini quiet">{% filter capfirst %}
                    {% trans entry.content_type.name %}{% endfilter %}</span>
                {% else %}
                  <span class="mini quiet">{% trans 'Contenido desconocido' %}</span>
                {% endif %}
              {% endspaceless %}&nbsp;
