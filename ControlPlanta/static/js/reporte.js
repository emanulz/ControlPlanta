...
//console.log($('#cajero').val());
    ...
    $("#BtnGenerarReporte").on("click",function(){
            if($("#fechainicial").val()!=''&&$("#fechafinal").val()!=''){
               ...
               alertify.alert('ERROR','Por favor seleccione valores de fechas válidos.')
            }

    });

    $("#BtnImprimirreporteventas").on("click",function(){
        event.preventDefault();
    ...
    $("#BtnImprimirreportegastos").on("click",function(){
        ...
        $( ".tituloreportegastos:hidden").show();
        $( "#reportegastos").printArea();
...
function GenerarReporteVentas(){
    ...
    $("#tablareporteventas > tbody").html("");
    totalreporteventas=0;
    ivreporteventas=0;
    ...
    if (ventasrango.responseJSON[i].anulada==true){
        ...
        estado=' ';
        totalreporteventas=totalreporteventas+ventasrango.responseJSON[i].total;
        ivreporteventas=ivreporteventas+ventasrango.responseJSON[i].iv;
    }

    $('#tablareporteventas > tbody:last').append('<tr><td>' + ventasrango.responseJSON[i].id + '</td><td>' + ventasrango.responseJSON[i].nombrecliente  +
    ...
    $(".reporteventas:hidden").show();
    $(".sidetotalesventa:hidden").show();

    $(".totalventas").html(totalreporteventas.toFixed(2));
    $(".totalivventas").html(ivreporteventas.toFixed(2));
...
function GenerarReporteGastos(){
    ...
    $("#tablareportegastos > tbody").html("");
    totalreportegastos=0;
    ...
    var gastosrango=$.get('/api/gasto/?min_date='+$("#fechainicial").val()+'&&max_date='+$("#fechafinal").val(),function(){});



    $.each( gastosrango.responseJSON, function(i){

    //console.log(ventasrango.responseJSON[i].id)
    var tipo=$.get('/api/tipogasto/'+gastosrango.responseJSON[i].tipo +'/',function(){});
    var factura='';
    if(gastosrango.responseJSON[i].factura==null){
        factura='Sin factura'
    }
    else{
        factura=gastosrango.responseJSON[i].factura
    }

    totalreportegastos=totalreportegastos+gastosrango.responseJSON[i].amount;


    $('#tablareportegastos > tbody:last').append('<tr><td>' + gastosrango.responseJSON[i].id + '</td><td>' + gastosrango.responseJSON[i].date  +
    '</td><td>' + tipo.responseJSON.name + '</td><td >' + factura +
    '</td><td>' + gastosrango.responseJSON[i].description +'</td><td class="price">' + gastosrango.responseJSON[i].amount.toFixed(2) +'</td></tr>');


    });

    var canalesrango=$.get('/api/canales/?min_date='+$("#fechainicial").val()+'&&max_date='+$("#fechafinal").val(),function(){});

    $.each( canalesrango.responseJSON, function(i){
    //console.log(ventasrango.responseJSON[i].id)
    var weight=canalesrango.responseJSON[i].weight;
    var preciokilo=canalesrango.responseJSON[i].preciokilo;
    var precio=weight*preciokilo;
    totalreportegastos=totalreportegastos+precio;


    $('#tablareportegastos > tbody:last').append('<tr><td>' + canalesrango.responseJSON[i].id + '</td><td>' + canalesrango.responseJSON[i].date  +
    '</td><td>' + 'Compra de canal' + '</td><td >' + '-' +
    '</td><td>' + 'Compra de canal' +'</td><td class="price">' + precio.toFixed(2) +'</td></tr>');


    });

    $(".reportegastos:hidden").show();
    $(".sidetotalesgastos:hidden").show();

    $(".totalgastos").html(totalreportegastos.toFixed(2));
